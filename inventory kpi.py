...
if activity_df is not None and map_df is not None:
    ...
    if len(date_range) == 2:
        filt = filt[(filt["Activity Date"] >= pd.to_datetime(date_range[0])) &
                    (filt["Activity Date"] <= pd.to_datetime(date_range[1]))]

    # ── Overall KPI tiles ─────────────────────────────────────────────────────
    st.markdown("### 🚀 Overall Performance Summary")
    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("Total Activities", len(filt))
    col2.metric("Excellent (<=2 d)", (filt["Performance"] == "Excellent").sum())
    col3.metric("Good (2<d<3)",     (filt["Performance"] == "Good").sum())
    col4.metric("Average (3≤d<4)",  (filt["Performance"] == "Average").sum())
    col5.metric("Need Improve (≥4 d)", (filt["Performance"] == "Need Improvement").sum())

    # rating helper
    def perf_rating(avg_delay):
        if pd.isna(avg_delay):
            return "Missing"
        elif avg_delay <= 2: return "Excellent"
        elif avg_delay < 3:  return "Good"
        elif avg_delay < 4:  return "Average"
        else:                return "Need Improvement"

    # ── Subordinate table ─────────────────────────────────────────────────────
    st.markdown("### 👥 Subordinate Performance")
    sub_df = (filt.groupby("subordinate")
                    .agg(Total_Activities=("Delay (Days)", "count"),
                         Average_Delay   =("Delay (Days)", "mean"))
                    .round(2)
                    .reset_index())
    sub_df["Rating"] = sub_df["Average_Delay"].apply(perf_rating)
    ...
    with col_b:
        ...
        qtr["Rating"] = qtr["Delay (Days)"].apply(perf_rating)
    ...
    with col_c:
        ...
        wk["Rating"] = wk["Delay (Days)"].apply(perf_rating)
    ...
    with col_d:
        ...
        daily["Rating"] = daily["Delay (Days)"].apply(perf_rating)
    ...
    st.warning("⚠️ PDF export coming soon – add `pdfkit` or `reportlab` integration here.")
else:
