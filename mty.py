...
type_options = ["Heavy Duty", "Hi-Cube", "Dry", "Special", "Flat Rack", "Open Top", "Reefer", "Standard"]
...
with tab1:
    ...
    st.download_button("Download MYT Summary as Excel", data=excel_myt_file, file_name='myt_summary.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')

    excel_filtered_myt_file = convert_df_to_excel(filtered_myt, include_index=False)
    st.download_button("Download Filtered MYT Data as Excel", data=excel_filtered_myt_file, file_name='filtered_myt_data.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
with tab2:
    ...
    # Dropdown for Type
    selected_type_on_the_way = st.selectbox("Select Type:", type_options, key='on_the_way_type')
    ...
    filtered_on_the_way = filtered_on_the_way[filtered_on_the_way['Type'] == selected_type_on_the_way]  # Filter by Type
    ...
    st.download_button("Download On The Way Summary as Excel", data=excel_on_the_way_file, file_name='on_the_way_summary.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')

    excel_filtered_on_the_way_file = convert_df_to_excel(filtered_on_the_way, include_index=False)
    st.download_button("Download Filtered On The Way Data as Excel", data=excel_filtered_on_the_way_file, file_name='filtered_on_the_way_data.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
with tab3:
    ...
    st.download_button("Download Utilized Summary as Excel", data=excel_utilized_file, file_name='utilized_summary.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')

    excel_filtered_utilized_file = convert_df_to_excel(filtered_utilized, include_index=False)
    st.download_button("Download Filtered Utilized Data as Excel", data=excel_filtered_utilized_file, file_name='filtered_utilized_data.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
